...
router.post('/', async (req, res) => {
    try {
        ...
        try {
            ...
            const created = await (0, fallbackOrders_1.fallbackCreateOrder)({
                ...order,
            });
            return res.json({
                success: true,
...
router.get('/', async (req, res) => {
    try {
        ...
        try {
            ...
            console.warn('Mongo unavailable, using fallback order store', dbError);
            const orders = await (0, fallbackOrders_1.fallbackGetOrders)(status);
            return res.json({ success: true, orders });
        }
    }
    catch (error) {
...
router.patch('/:id', async (req, res) => {
    try {
        ...
        try {
            const { db } = await (0, db_1.connectToDatabase)();
            const result = await db.collection('orders').updateOne({ _id: new mongodb_1.ObjectId(id) }, {
                $set: {
                    ...body,
                    updatedAt: new Date(),
                },
            ...
            console.warn('Mongo unavailable, using fallback order store', dbError);
            const updated = await (0, fallbackOrders_1.fallbackUpdateOrder)(id, {
                ...body,
            });
            if (!updated)
                return res.status(404).json({ success: false, error: 'Order not found' });
...
router.delete('/:id', async (req, res) => {
    try {
        ...
        try {
            const { db } = await (0, db_1.connectToDatabase)();
            const result = await db.collection('orders').deleteOne({ _id: new mongodb_1.ObjectId(id) });
            ...
            console.warn('Mongo unavailable, using fallback order store', dbError);
            const deleted = await (0, fallbackOrders_1.fallbackDeleteOrder)(id);
            if (!deleted)
                return res.status(404).json({ success: false, error: 'Order not found' });
            return res.json({ success: true });
        }
...
router.get('/track', async (req, res) => {
    try {
        ...
        try {
            ...
            console.warn('Mongo unavailable, using fallback order store', dbError);
            const order = await (0, fallbackOrders_1.fallbackFindByOrderNumber)(String(orderNumber));
            if (!order)
                return res.status(404).json({ success: false, error: 'Order not found' });
            return res.json({
                success: true,
