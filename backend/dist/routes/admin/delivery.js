...
router.get('/', async (_req, res) => {
    try {
        ...
        const areas = await (0, fallbackStore_1.fallbackGetDeliveryAreas)();
...
router.post('/', async (req, res) => {
    try {
        ...
        const body = req.body || {};
        const next = await (0, fallbackStore_1.fallbackUpsertDeliveryArea)({
...
router.patch('/', async (req, res) => {
    try {
        ...
        const { db } = await (0, db_1.connectToDatabase)();
        await db.collection('delivery_areas').updateOne({ _id: new mongodb_1.ObjectId(id) }, { $set: { ...updates, updatedAt: new Date() } });
        ...
        const current = (await (0, fallbackStore_1.fallbackGetDeliveryAreas)()).find((a) => a._id === String(id));
        if (!current)
...
router.delete('/', async (req, res) => {
    try {
        ...
        const { db } = await (0, db_1.connectToDatabase)();
        await db.collection('delivery_areas').deleteOne({ _id: new mongodb_1.ObjectId(String(id)) });
        return res.json({ success: true });
    }
    catch (error) {
        ...
        if (!id)
            return res.status(400).json({ success: false, error: 'ID required' });
        const deleted = await (0, fallbackStore_1.fallbackDeleteDeliveryArea)(String(id));
        if (!deleted)
            return res.status(404).json({ success: false, error: 'Area not found' });
        return res.json({ success: true });
    }
